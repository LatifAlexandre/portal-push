...
public class PortalPushUIInitHandler extends PortletUIInitHandler {
	...
	protected String getInitialUidl(VaadinRequest request, UI uI) throws IOException {
		...
		System.out.println("PortalPushUIInitHandler.getInitialUidl()");
		PortletRequest pr = ((VaadinPortletRequest) request).getPortletRequest();
		// Send push requests to our servlet
		// uI.getPushConfiguration().setPushUrl(pr.getContextPath() + "/PUSH");
		uI.getPushConfiguration().setPushUrl("/o/PUSH");
		System.out.println("PortalPushUIInitHandler.getInitialUidl() - " + uI.getPushConfiguration().getPushUrl());
